...
class InfluxDBDump:
    ...
    def get_tag_values(self, tagname, metric):
        result = []
        query = "SHOW TAG VALUES FROM /^" + metric +".*/ WITH KEY = \"" + tagname +"\""
    ...
    def get_jvms(self, metric):
        return self.get_tag_values("jvmName", metric)

    def get_hosts(self, metric):
        return self.get_tag_values("hostname", metric)

    def output_to_file(self, out_filename, tags, metric):
        print("=== Making file %s" % out_filename)
        clauses = ["%s ='%s'" % (tag, value) for (tag, value) in tags.items()]
        query = 'select value from /^%s.*/ where %s' % (metric, " and ".join(clauses))
        print("running query: %s" % query)
        ...
        for metric in series:
            with open(out_filename + metric['name'], "w") as f:
    ...
    def run(self):
        ...
        for jvm in jvms:
            ...
            tags["jvmName"] = jvm
            clauses = ["%s ='%s'" % (tag, value) for (tag, value) in tags.items()]
            query = 'select value from /^%s.*/ where %s limit 1' % (metric, " and ".join(clauses))
            print("======== %s ======== " % jvm)
            print("running query: %s" % query)
            ...
            self.output_to_file(filename, tags, metric)
            print("")

        # dump files for every host
        hosts = self.get_hosts(metric)
        for host in hosts:
            # run a query to find out the date and time when measurements were started
            tags = dict(self.mapped_tags)
            tags["hostname"] = host
